...
import core.{
  type Category, type Post, type PostFormData, PageOwnedPosts, Post,
  PostFilterSubmitted, SlugGeneratorClicked,
}
import ffi
...
pub fn render_post_edit_page(_id: String, model: Model) {
  ...
  case is_loading {
    ...
    False -> {
      ...
      element.fragment([
        skeleton.render_header_bar(core.LogOutClicked),
        skeleton.render_main_block([form], ""),
      ])
    }
  }
}

fn render_post_form(post: Option(Post), _form: formlib.Form(PostFormData)) {
  ...
  let class_text_input =
    "py-2 w-full text-gray-700 bg-white border rounded-md sm:mx-2 dark:bg-gray-900 dark:text-gray-300 dark:border-gray-600 focus:border-blue-400 dark:focus:border-blue-300 focus:outline-none focus:ring focus:ring-blue-300 focus:ring-opacity-40"
