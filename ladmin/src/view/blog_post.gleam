...
pub fn render_post_table_page(
  ...
  case loading_status {
    ...
    _ -> {
      let posts = case model.page_owned_objects {
        PageOwnedPosts(objects) -> objects
        _ -> []
      }
      let total_pages = model.page_owned_object_paging.total_pages
      let query_list = []
      let query_list = case q {
        Some(query) -> [#("q", query), ..query_list]
        _ -> query_list
      }
      let query_list = case cat_id {
        Some(id) -> [#("cat_id", id), ..query_list]
        _ -> query_list
      }
      ...
      h.div([a.class("flex items-center space-x-4")], [
        ...
        h.button(
          [
            ...
            ev.on_click(deletion_click_handler(post.id)),
          ],
          [
...
fn render_filter_form(
  ...
  let select_handler = {
    ...
    form_data
    |> result.map(fn(data) { decode.success(core.PostFilterSubmitted(data)) })
    |> result.lazy_unwrap(fn() {
      decode.failure(core.PostFilterSubmitted([]), "FormData")
    })
  }
...
pub fn render_post_edit_page(id: String, model: Model) {
  ...
  case loading_status {
    ...
    _ -> {
      let Model(users:, loading_status:, draft_post:, ..) = model
